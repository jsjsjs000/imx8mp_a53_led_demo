...
static void execute_commands(const char *command, char *result)
{
	...
	if (strcmp("leds", command) == 0)
	{
		...
		int8_t led_r_ = parameters[0];
		int8_t led_g_ = parameters[1];
		int8_t led_b_ = parameters[2];
		enum led_mode_t led_mode_ = parameters[3];
    if (led_r_ != led_r || led_g_ != led_g || led_b_ != led_b || led_mode_ != led_mode)
    {
      led_r = led_r_;
      led_g = led_g_;
      led_b = led_b_;
